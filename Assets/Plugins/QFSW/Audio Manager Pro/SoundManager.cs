...
namespace AMPInternal
{
    ...
    public abstract class SoundManager : MonoBehaviour
    {
        ...
        public  void StopAllWithFade()
        {
            foreach (AudioObject audioObject in ActiveAudio)
            {
                DOTween.To(() => audioObject.volume,
                    x => audioObject.volume = x,0, 0.7f).OnComplete(() =>
                {
                    audioObject.Stop();
                    InactiveAudio.Add(audioObject);
                });
            }
        }
        ...
        public AudioObject StartAudio(AudioClip Audio, bool Looping, bool AutoPlay = true)
        {
            ...
            if (InactiveAudio.Contains(ReturnedAudio))
            {
                InactiveAudio.Remove(ReturnedAudio);
            }
            ...
            ReturnedAudio.Source.loop = IsLoop;
            ReturnedAudio.Source.clip = Audio;
            ReturnedAudio.Source.priority = 128;
            if (IsLoop)
            {
                DOTween.To(() => ReturnedAudio.volume,
                    x => ReturnedAudio.volume = x, 1, 1f);
            }
            else
            {
